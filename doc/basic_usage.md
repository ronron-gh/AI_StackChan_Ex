...
### 3.1. メモリー（長期記憶）について
メモリーを有効にするには SDカードの/app/AiStackChanEx/SC_ExConfig.yaml で enableMemory を true に設定してください。ChatGPT（Realtime API含む）のみ対応です。
...
> 記憶するかどうかはLLMが判断するため、ユーザーの期待通りに記憶されない場合があります。また、一度記憶した情報も、要約を繰り返す過程で失われる可能性があります。
