...
void perform_idle_motion() {
  ...
  // 他のアクション（会話・頭撫で）中はスキップ
  if (!servo_home) return;
...
void idle_motion_tick() {
  if (s_paused || !robot) return;
  uint32_t now = millis();
  if (now < s_next_tick_ms) return;

  // サーボが他の動きで使用中なら 500ms 後にリトライ
  if (!servo_home) {
